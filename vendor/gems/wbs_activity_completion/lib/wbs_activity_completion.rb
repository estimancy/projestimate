...
module WbsActivityCompletion
  ...
  class WbsActivityCompletion
    attr_accessor :pbs_project_element, :inputs_effort_person_hour
    ...
    def initialize(module_input_data)
      ...
      @inputs_effort_person_hour = module_input_data[:effort_person_hour]
    end

    # The Output result
    def get_effort_person_hour(*args)
      output_effort_person_hour = Hash.new
      @inputs_effort_person_hour.each {|key, value| output_effort_person_hour[key.to_i] = value.blank? ? nil : value.to_f}
      output_effort_person_hour
    end
  end
end
