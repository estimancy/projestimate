...
class ProjectsController < ApplicationController
  ...
  def dashboard
    ...
    if @module_project.pemodule.alias == "expert_judgement"
      ...
      @kb_input = Kb::KbInput.where(module_project_id: current_module_project.id).last
      @project_list = []
...
=begin
    ...
    worksheet.sheet_name = "Projet"
    worksheet.add_cell(0, 0, "Nom du projet")
    worksheet.add_cell(0, 1, project.title)
    worksheet.add_cell(1, 0, "Version")
    worksheet.add_cell(1, 1, project.version)
    worksheet.add_cell(2, 0, "Date de lancement")
    worksheet.add_cell(2, 1, project.start_date)
    worksheet.add_cell(3, 0, "Model d'origine")
    worksheet.add_cell(3, 1, Project.where(id:  project.original_model_id).first.title)
    worksheet.add_cell(4, 0, "Description")
    worksheet.add_cell(4, 1, project.description)
    worksheet.add_cell(5, 0, "Organisation")
    worksheet.add_cell(5, 1, Organization.where(id: project.organization_id).first.name)
    worksheet.add_cell(5, 0, "Etat")
    worksheet.add_cell(5, 1, EstimationStatus.where(id: project.estimation_status_id).first.name)
    worksheet.add_cell(6, 0, "Secteur de porjets")
    worksheet.add_cell(6, 1, project.project_area.nil? ? "N/A" : project.project_area)
    worksheet.add_cell(7, 0, "Categorie d'acquisition")
    worksheet.add_cell(7, 1, project.acquisition_category.nil? ? "N/A" : project.acquisition_category)
    worksheet.add_cell(8, 0, "Categorie de plate-forme")
    worksheet.add_cell(8, 1, project.platform_category.nil? ? "N/A" : project.platform_category)
    worksheet.add_cell(9, 0, "Categorie de projet")
    worksheet.add_cell(9, 1, project.project_category.nil? ? "N/A" : project.project_category)

    workbook.add_worksheet("Plan d'estimation")
    worksheet = workbook[1]
    project.module_projects.each_with_index do |mp, i|
      case mp.pemodule.alias
        when "guw"
          worksheet.add_cell(i,1, mp.guw_model)
        when "kb"
          worksheet.add_cell(i,1, mp.kb_model)
      end
    end
=end
...
#    system "rake db:data:dump"
    #send_data(workbook.stream.string, filename: "#{project.title.gsub(" ", "_")}-#{Time.now.strftime("%m-%d-%Y_%H-%M")}.xlsx", type: "application/vnd.ms-excel")
